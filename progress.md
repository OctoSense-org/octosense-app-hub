...
- Refreshed Octoscript-Makepad branch `feat/app-card-runtime` adds host-owned native callbacks and stateful L0 preparation at `4de95e1`; its targeted and full package tests passed. Hub now pins that revision, and native preparation accepts portable L0 nodes while preserving the existing native-kit path.
- Installed Card, reference host and validator now use one `CardSession`. It creates an unpredictable event channel, grants only `agent.notify` for generic L0 controls, checks current-tree event identity in `octosense-app-runtime`, and re-renders after a declared transition. A native validator fixture loaded the real `OctoscriptTap` widget; its event changed displayed text from `off` to `on`, while wrong-channel and undeclared events were ignored. All 10 native validator integration tests and the full Hub workspace suite passed. Physical input/device evidence, durable state, async effects and `app.logic@1` remain.
- Refreshed Octoscript main at `f25070a` was used for a versioned, bounded `InstanceStore` snapshot API; its full L0 suite passed and commit `3c1c6ba` was pushed. Octoscript-Makepad now pins that revision at `4fe5cc5`; its full makepad package suite passed. Hub pins both published revisions instead of masking them with old sibling checkouts.
- The shared Card session now saves storage-granted state to a host-owned file, outside the app's writable jail. Writes are atomic replacement with file sync, bounded by 1 MiB and the declared app storage quota. A restart restores the displayed value; an old generation/channel cannot change it. Failed saves roll back the in-memory transition and leave the current UI in place. The new persistence test and full Hub workspace suite passed. Physical tap/device evidence, incompatible-schema migration, async effects, combined jail-plus-snapshot quota accounting and `app.logic@1` remain.
- Hub commit `7df6ff3` and the matching System Apps Mail service pin `0a60005` are published on feature branches. Active ROM Home commit `008098f` pins that Hub and removes stale local Octoscript/Makepad overlays; its full shell check and 310 library tests passed against fetched exact Git revisions, with the pre-existing `app-finance`/`app-robrix` cfg warnings. No device rollout.
- Plan 06/07 next slice: a durable collection write or stale source now refuses the entire L0 transition until a service can settle it; the regression failed before the fix and passes after. The existing Splash host bridge already drained contained requests. The shared App Store broker now bounds pending work per heap/process, delivers a 30-second timeout, ignores late responses after timeout/close, checks request policy at drain, caps JSON arguments and answers, and denies prompts from background surfaces. Mail's add-account path checks host-owned prompt permission before retaining a pending sign-in. Full affected Hub suites and cross-repo Mail/Home integration are being revalidated.
- Standalone Mail compilation exposed the old Hub Makepad manifest revision `7786bb4` alongside the renderer's refreshed `cd812ac`, a type mismatch hidden by the Hub's local workspace patch. Every Hub Makepad manifest now pins `cd812ac`, and the Hub build uses exact published Git sources without local Makepad overrides. The locked offline workspace check passes with this graph; full tests and cross-repo integration are being rerun.
